function nabla_f = gradient_mat_per(f,dX)
...
if d == 2
    ...
    nabla_f(:,:,1,:,1) = 1/(2*dx) * (f(:,:,2,:)-f(:,:,end,:));
    nabla_f(:,:,end,:,1) = 1/(2*dx) * (f(:,:,1,:)-f(:,:,end-1,:));
    
    % Compute the gradient along y
    dy=dX(2);
    nabla_f(:,:,:,2:end-1,2) = 1/(2*dy) * (f(:,:,:,3:end)-f(:,:,:,1:end-2));
    nabla_f(:,:,:,1,2) = 1/(2*dy) * (f(:,:,:,2)-f(:,:,:,end));
    nabla_f(:,:,:,end,2) = 1/(2*dy) * (f(:,:,:,1)-f(:,:,:,end-1));
    ...
    nabla_f(:,:,:,:,1,3) = 1/(2*dz) * (f(:,:,:,:,2)-f(:,:,:,:,end));
    nabla_f(:,:,:,:,end,3) = 1/(2*dz) * (f(:,:,:,:,1)-f(:,:,:,:,end-1));
end
